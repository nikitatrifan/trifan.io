...
//import GlitchedSlider from "./GlitchedSlider";
...
class MatrixRainingCode extends React.Component {
  ...
  animate = () => {
    if (this.isAnimationEnded) return false;
    ...
    this.columns.forEach((block, idx) => {
      ...
      setTimeout(() => {
        if (this.isAnimationStopped) return false;
  ...
  startImageAnimation = isEnded => {
    if (isEnded) {
      return TweenMax.to(this.image, 0.3, {
        filter: "blur(0px)",
        scale: 1,
  ...
  componentDidMount() {
    if (this.props.noAnimation) return false;
          ...
          columns.map(it => (
            ...
            >
              <div className={classes.col_wrapper}>
                {rows.map(idx => (
                  <span className={classes.code} key={idx}>
...
const styles = {
  ...
  image: {
    ...
    zIndex: 0,
    filter: `blur(${MatrixRainingCode.initialImageBlur})`,
