...
export default class GlitchScene {
    constructor(canvas) {
        this.isKilled = false;
    ...
    resizeWindow = () => {
        const {
            canvas, cameraBack, bgImg,
            postEffect, renderBack1, renderer
        } = this;
    ...
    renderLoop = () => {
        if (this.isKilled || this.isStopped)
            return false;
    ...
    attachResize = () =>
        window.addEventListener('resize', this.resize, 1000);
    detachResize = () =>
        window.removeEventListener('resize', this.resize, 1000);

    appendImage = (image, cb) => {
        const { bgImg, sceneBack, postEffect, scene } = this;
        bgImg.init(image, () => {
    ...
    initialize = (image, cb) => {
        if (this.isInitialized) {
            return this.appendImage(image, cb);
        }

        this.isInitialized = true;

        const { renderer, cameraBack } = this;
        renderer.setSize(window.innerWidth, window.innerHeight);
